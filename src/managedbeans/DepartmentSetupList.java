...
@CCGenClass (expressionBase="#{d.DepartmentSetupList}")

public class DepartmentSetupList extends PageBean implements Serializable{
	private Rec_department_setup listModel = new Rec_department_setup();
	...
	public void build(){
		try{
			...
			//TODO Get Table Data
			List<Rec_department_setup> results = Rec_department_setup.findAll();
			for(Rec_department_setup result : results){
				MyRow row = new MyRow();
...
//			m_rowBind.ensureItemToBeDisplayed(myRow);
	...
	private void popupCard(ArrayList<String> row, boolean isEnabled){
		//Write code to pop up card
        /*try{
            DepartmentSetupList popupCardVar = new DepartmentSetupList();
            ...
            ModalPopup popup = openModalPopup(popupCardVar, "DepartmentSetupList Card", 0, 0,
            ...
            popup.maximize(true);
            popupCardVar.prepare(new DepartmentSetupList.ICloseListener() {
                @Override
	...
	public DepartmentSetupList(){
	}
	public String getPageName() { return "/DepartmentSetupList.jsp"; }
	public String getRootExpressionUsedInPage() { return "#{d.DepartmentSetupList}"; }
