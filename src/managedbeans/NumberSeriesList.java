...
@CCGenClass (expressionBase="#{d.NumberSeriesList}")

public class NumberSeriesList extends PageBean implements Serializable{
	private Rec_no_series listModel = new Rec_no_series();
	...
	public class MyRow extends FIXGRIDItem implements java.io.Serializable{
		...
		String varDefault_Nos_;
		public String getVarDefault_Nos_(){return varDefault_Nos_;}
		public void setVarDefault_Nos_(String value){this.varDefault_Nos_ = value;}

		String varManual_Nos_;
		public String getVarManual_Nos_(){return varManual_Nos_;}
		public void setVarManual_Nos_(String value){this.varManual_Nos_ = value;}

		String varDate_Order;
		public String getVarDate_Order(){return varDate_Order;}
		public void setVarDate_Order(String value){this.varDate_Order = value;}

		String varDist_Location_Prefix;
		public String getVarDist_Location_Prefix(){return varDist_Location_Prefix;}
		public void setVarDist_Location_Prefix(String value){this.varDist_Location_Prefix = value;}
		...
		public void onRowSelect() {
			if(m_rowBind.getItems().indexOf(m_rowBind.getSelectedItem()) != m_rowBind.getItems().indexOf(currentRecord)) {
				if(currentRecord == null) {
					...
					currentRecord = m_rowBind.getSelectedItem();
					if(xRecord.getEnabled() == true) {
					    if (!xRecord.getVarCode().isEmpty()) {
			...
			if(m_rowBind.getItems().size() != 0){
				if(m_rowBind.isItemSelected()) {
					if(m_rowBind.getSelectedItems().size() != 1){
                        ...
                        if (!currentRecord.getVarCode().isEmpty()) {
    ...
    public void onDetail(ActionEvent event){
        try{
            if (m_rowBind.isItemSelected()) {
                if (!m_rowBind.getSelectedItem().getVarCode().isEmpty()) {
                    NumberSeriesLineList lineList = new NumberSeriesLineList();

                    lineList.prepare(new NumberSeriesLineList.IListener() {
                        @Override
                        public String code() {
                            return m_rowBind.getSelectedItem().getVarCode();
                        }
                    });

                    lineList.setObjectID(457);
                    lineList.initProperties();
                    lineList.build();

                    ModalPopup modalPopup = openModalPopup(lineList, "Number Series Line", 0, 0,
    ...
    public void onRelationships(ActionEvent event){
        try{
	...
	public void build(){
		try{
			...
			//TODO Get Table Data
			List<Rec_no_series> results = Rec_no_series.findAll();
			for(Rec_no_series result : results){
				MyRow row = new MyRow();
				...
				row.setVarDefault_Nos_(tool.intToBooleanToString(result.getInteger(fieldNameArray.get(3))));
				row.setVarManual_Nos_(tool.intToBooleanToString(result.getInteger(fieldNameArray.get(4))));
				row.setVarDate_Order(tool.intToBooleanToString(result.getInteger(fieldNameArray.get(5))));
				row.setVarDist_Location_Prefix(tool.intToBooleanToString(result.getInteger(fieldNameArray.get(6))));
				m_rowBind.getItems().add(row);
			}
			...
			for(Rec_sys_obj_prop_table_fieldcaption fieldProperty : fieldProperties2){
				...
				case "ComboBox":
					...
					break;
				case "ComboField":
					xml.append("<t:combofield activationhotkey = 'alt-40' enabled='.{enabled}' actionListener = '#{d.NumberSeriesList.onComboField"+fieldProperty.getString("fieldname")+"}' text='" + ".{" + variableArray.get(col) + "}" + "'></t:combofield>");
					break;
				}
				xml.append("</t:gridcol>");
				col++;
	...
	private ArrayList<String> currentRowArray(){
		...
		arrayList.add(tool.booleanToString(currentRecord.getVarDefault_Nos_()));
		arrayList.add(tool.booleanToString(currentRecord.getVarManual_Nos_()));
		arrayList.add(tool.booleanToString(currentRecord.getVarDate_Order()));
		arrayList.add(tool.booleanToString(currentRecord.getVarDist_Location_Prefix()));
		return arrayList;
	}
	...
	private ArrayList<String> pastRowArray(){
		...
		arrayList.add(tool.booleanToString(xRecord.getVarDefault_Nos_()));
		arrayList.add(tool.booleanToString(xRecord.getVarManual_Nos_()));
		arrayList.add(tool.booleanToString(xRecord.getVarDate_Order()));
		arrayList.add(tool.booleanToString(xRecord.getVarDist_Location_Prefix()));
		return arrayList;
	}
...
//			m_rowBind.ensureItemToBeDisplayed(myRow);
	...
	private void popupCard(ArrayList<String> row, boolean isEnabled){
		//Write code to pop up card
        /*try{
            NumberSeriesList popupCardVar = new NumberSeriesList();
            ...
            ModalPopup popup = openModalPopup(popupCardVar, "NumberSeriesList Card", 0, 0,
                    new ModalPopup.IModalPopupListener() {
                        @Override
                        public void reactOnPopupClosedByUser() {
            ...
            popup.maximize(true);
            popupCardVar.prepare(new NumberSeriesList.ICloseListener() {
                @Override
	...
	private MyRow emptyRow(){
		...
		row.setVarDefault_Nos_("0");
		row.setVarManual_Nos_("0");
		row.setVarDate_Order("0");
		row.setVarDist_Location_Prefix("0");
		return row;
	}
	...
	public NumberSeriesList(){
	}
	public String getPageName() { return "/NumberSeriesList.jsp"; }
	public String getRootExpressionUsedInPage() { return "#{d.NumberSeriesList}"; }
