...
@CCGenClass (expressionBase="#{d.ScheduleHeaderList}")

public class ScheduleHeaderList extends PageBean implements Serializable{
	private Rec_schedule_header listModel = new Rec_schedule_header();
	...
	public class MyRow extends FIXGRIDItem implements java.io.Serializable{
		...
		String varCutoff_fromDate;
		public String getVarCutoff_fromDate(){return varCutoff_fromDate;}
		public void setVarCutoff_fromDate(String value){this.varCutoff_fromDate = value;}

		String varCutoff_toDate;
		public String getVarCutoff_toDate(){return varCutoff_toDate;}
		public void setVarCutoff_toDate(String value){this.varCutoff_toDate = value;}
	...
	public void onNew(javax.faces.event.ActionEvent event) {
		if(m_attrCardPageID != 0){
			if(m_attrInsertAllowed) {
				...
				popupDocument(emptyRowArray(), true);
			}else{
	...
	public void onEdit(javax.faces.event.ActionEvent event) {
		if(m_attrCardPageID != 0){
			if(m_rowBind.getItems().size() != 0){
				if(m_attrModifyAllowed) {
					if(m_rowBind.isItemSelected()){
						...
						popupDocument(currentRowArray(), true);
					}else{
			...
			if(m_rowBind.getItems().size() != 0){
				if(m_attrModifyAllowed) {
					if(m_rowBind.isItemSelected()) {
						if(!m_rowBind.getSelectedItem().getEnabled()) {
	...
	public void onView(javax.faces.event.ActionEvent event) {
		if(m_attrCardPageID == 0) {
			...
			if(m_rowBind.getItems().size() != 0){
				if(m_rowBind.isItemSelected()){
					popupDocument(currentRowArray(), false);
				}else{
					Statusbar.outputError("No Selection Of Item Detected.");
				}
			}else{
	...
	public void build(){
		try{
			...
			//TODO Get Table Data
			List<Rec_schedule_header> results = Rec_schedule_header.findAll();
			for(Rec_schedule_header result : results){
				MyRow row = new MyRow();
				...
				row.setVarCutoff_fromDate(tool.formatSQLDate(result.getDate(fieldNameArray.get(3))));
				row.setVarCutoff_toDate(tool.formatSQLDate(result.getDate(fieldNameArray.get(4))));
				m_rowBind.getItems().add(row);
			}
			...
			for(Rec_sys_obj_prop_table_fieldcaption fieldProperty : fieldProperties2){
				...
				case "ComboBox":
					...
					break;
				case "ComboField":
					xml.append("<t:combofield  editable = 'false' activationhotkey = 'alt-40' enabled='.{enabled}' actionListener = '#{d.ScheduleHeaderList.onComboField"+fieldProperty.getString("fieldname")+"}' text='" + ".{" + variableArray.get(col) + "}" + "'></t:combofield>");
					break;
				}
				xml.append("</t:gridcol>");
				col++;
	...
	private ArrayList<String> currentRowArray(){
		...
		arrayList.add(currentRecord.getVarCutoff_fromDate());
		arrayList.add(currentRecord.getVarCutoff_toDate());
		return arrayList;
	}
	...
	private ArrayList<String> pastRowArray(){
		...
		arrayList.add(xRecord.getVarCutoff_fromDate());
		arrayList.add(xRecord.getVarCutoff_toDate());
		return arrayList;
	}
...
//			m_rowBind.ensureItemToBeDisplayed(myRow);
	...
	private void popupDocument(ArrayList<String> row, boolean isEnabled){
		//Write code to pop up card
        try{
            ScheduleHeaderDocument popupCardVar = new ScheduleHeaderDocument();
            popupCardVar.setEnabled(isEnabled);
            popupCardVar.setListObjID(objectID);
            popupCardVar.setObjectID(300010);
            popupCardVar.setValues(row);
            popupCardVar.builddocument();

            ModalPopup popup = openModalPopup(popupCardVar, "ScheduleHeaderList Document", 0, 0,
            ...
            popup.maximize(true);
            popupCardVar.prepare(new ScheduleHeaderDocument.ICloseListener() {
                @Override
	...
	private MyRow emptyRow(){
		...
		row.setVarCutoff_fromDate(dateFormat.format(date));
		row.setVarCutoff_toDate(dateFormat.format(date));
		return row;
	}
	...
	public ScheduleHeaderList(){
	}
	public String getPageName() { return "/ScheduleHeaderList.jsp"; }
	public String getRootExpressionUsedInPage() { return "#{d.ScheduleHeaderList}"; }
