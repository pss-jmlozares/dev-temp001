...
@Table("position_setup")
@CompositePK("Code")
public class Rec_position_setup extends Model{
	...
	private String preInsert(int listObjID, String code){
		try{
			...
			Rec_position_setup record = new Rec_position_setup();
	...
	private boolean onInsert(ArrayList<String> row, int listObjID) {
		try{
			...
			Rec_position_setup rec = Rec_position_setup.findFirst("Code = ?", code);
	...
	private boolean onModify(ArrayList<String> row, int listObjID){
		try{
			...
			Rec_position_setup rec = Rec_position_setup.findFirst("Code = ?", row.get(1));
			for(int count = 2; count < captionArray.size(); count++) {
				rec.set(captionArray.get(count), row.get(count));
			}
			rec.saveIt();
