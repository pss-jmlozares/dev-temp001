...
@Table("posted_calculate_table")
@CompositePK("id")
public class Rec_posted_calculate_table extends Model{
	...
	private String preInsert(int listObjID, String code){
		try{
            ...
            String Department = ""; 
            String Position = ""; 
            String Actual_morning_IN = "17:29:10"; 
            String Scheduled_morning_IN = "17:29:10"; 
            String Actual_morning_OUT = "17:29:10"; 
            String Scheduled_morning_OUT = "17:29:10"; 
            String Actual_afternoon_IN = "17:29:10"; 
            String Scheduled_afternoon_IN = "17:29:10"; 
            String Actual_afternoon_OUT = "17:29:10"; 
            String Actual_afternoon_IN_copy1 = "17:29:10"; 
            String Late = "0"; 
            String Absent = "0"; 
            String UnderTime = "0"; 
            String OverTime = "0"; 
			...
			array.add(Actual_afternoon_IN_copy1);
			array.add(Late);
			array.add(Absent);
			array.add(UnderTime);
			array.add(OverTime);
			Rec_posted_calculate_table record = new Rec_posted_calculate_table();
	...
	private boolean onInsert(ArrayList<String> row, int listObjID) {
		try{
			...
			Rec_posted_calculate_table rec = Rec_posted_calculate_table.findFirst("id = ?", code);
			for(int count = 2; count < captionArray.size(); count++) {
				rec.set(captionArray.get(count), row.get(count));
			}
			rec.saveIt();

			List<Rec_calculate_table> ctl = Rec_calculate_table.findAll();
			for (Rec_calculate_table list:ctl){
			    list.delete();
            }
	...
	private boolean onModify(ArrayList<String> row, int listObjID){
		try{
			...
			Rec_posted_calculate_table rec = Rec_posted_calculate_table.findFirst("id = ?", row.get(1));
			for(int count = 2; count < captionArray.size(); count++) {
				rec.set(captionArray.get(count), row.get(count));
			}
			rec.saveIt();
